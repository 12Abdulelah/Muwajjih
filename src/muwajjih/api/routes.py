...
from muwajjih.api.schemas import PredictRequest, PredictionData, ResponseEnvelope
...
def health(request: Request) -> ResponseEnvelope:
    return ResponseEnvelope(trace_id=request.state.trace_id, data={"status": "alive"}, error=None)
...
def predict(
    ...
    result = service.triage(Complaint(text=payload.complaint), request.state.trace_id)
    data = PredictionData(department=result.department, priority=result.priority)
    return ResponseEnvelope(trace_id=request.state.trace_id, data=data, error=None)
